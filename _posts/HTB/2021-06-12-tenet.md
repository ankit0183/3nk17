...
excerpt: "Tenet is a medium rated machine on HackTheBox created by [egotisticalSW](https://www.hackthebox.eu/home/users/profile/94858). For the user part we will find a backup php script and abuse the php unserialze function to reach RCE on the webserver resulting in a reverse shell. Next we find database credentials, with wich we can switch to the user Neil. Finally we will exploit a race condition in a bash script we are able to run as root to write our public ssh key to root's authorized ssh keys."
image: /img/tenet/info_card.png
---
